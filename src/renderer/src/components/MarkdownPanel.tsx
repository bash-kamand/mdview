...
function buildComponents(onOpenExternal: (url: string) => void): Components {
  return {
    a({ href, children }) {
      return (
    ...
    input({ type, checked, ...props }) {
      if (type === 'checkbox') {
        return <input type="checkbox" checked={checked} readOnly style={{ pointerEvents: 'none' }} {...props} />
      }
...
function CopyButton({ onCopy }: { onCopy: () => void }) {
  ...
  return (
    <PanelButton onClick={handleCopy} title="Copy file content">
      {copied ? (
...
export default function MarkdownPanel({
      ...
      <div className="flex flex-col items-center justify-center h-full text-center px-8 gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500">
            Browse CLAUDE.md, memory files, slash commands, and more.
          </p>
        </div>
        <button onClick={onOpenFolder} className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md text-sm">
          Open Folder
        </button>
      </div>
    )
  ...
  if (!hasFiles) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-gray-400 dark:text-gray-500 text-sm">No markdown files found in this folder</p>
      </div>
    )
  }
  ...
  if (!content) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-gray-300 dark:text-gray-600 text-sm">Select a file to view</p>
      </div>
    )
  }

  const components = buildComponents(onOpenExternal)
  ...
  return (
    <div className="flex h-full min-h-0">
      ...
      <div className="flex flex-col flex-1 min-w-0 min-h-0">
        {/* Panel header */}
        <div className="flex items-center gap-1.5 px-3 py-1.5 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 flex-shrink-0">
          <span className="text-xs font-mono text-gray-400 flex-1 truncate min-w-0">
